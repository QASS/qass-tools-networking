Overview
********

The :class:`AnalyzerSSH` class extends the possible remote control of an Optimizer4D via python context. The provided methods are msotly designed to extract characteristical values to identify hardware type and status of connected machine.
But can be used to send any ssh command, by using Paramiko channel. Be aware that every time a command is getting executed a new channel is opend (old one closes).

ErrorHandling
"""""""""""""
The module contains three kinds of custom exception classes. :class:`BadConnection` is a Exception class from Paramiko module. :class:`BadPswdWxception` is only raised by occuring timeout after trying to send terminal a sudo apssword which is not accepted.
When a sudo password is required but missing (not set beforehand) a :class:`MissingPswdError` is raised.  

Examples
********

Example 1: Auto read all information
""""""""""""""""""""""""""""""""""""
Simple example for examine all supported hardware information and store them as local JSON File.

.. code-block:: python
   :linenos:

        with SSH.SSHConnector("machine_IP", ssh_password="ssh_password") as terminal:
            terminal.set_sudo_password("sudo_host_password")
            res = terminal.get_hardware_information()
            terminal.export_to_json(res)
            # please see attached JSON file in example directory

Example 2: Handle results by stdout and stderr
""""""""""""""""""""""""""""""""""""""""""""""
""" Simple example for analyzing an empty result without exception."""  

.. code-block:: python
   :linenos:

        with SSH.SSHConnector("machine_IP", ssh_password="ssh_password") as terminal:
            terminal.set_sudo_password("sudo_host_password")
            #res = terminal.get_hardware_information()
            #terminal.export_to_json(res)
            out, outerr = terminal.send_command("lt", give_out_stderr=True)
            print("stdout:",out) # terminal output--> 'stdout: []'
            print("stderr:",outerr) # terminal output --> 'stderr: ['zsh:1: command not found: lt\n']'

Example 3: Examined specific data
"""""""""""""""""""""""""""""""""
""" Simple example for partially examined data."""

.. code-block:: python
   :linenos:

        with SSH.SSHConnector("machine_IP", ssh_password="ssh_password") as terminal:
            terminal.set_sudo_password("sudo_host_password")
            terminal.get_BIOS_information()
            device_paths = terminal.detect_harddrives()
            for device in device_paths:
                desired_inforamtion = terminal.get_harddrive_information(device, attributes=['SATA_version', 'size'])

AnalyzerSSH
************
.. autoclass:: qass.tools.networking.analyzer_ssh.AnalyzerSSH
        :members: